...
class AnalyticsService:
    ...
    def get_performance_summary(user_id):
        ...
        trades = db.get_recent_trades_summary(user_id, limit=500)
        ...
        win_rate = (
            len([t for t in trades if float(t.get('result', 0)) > 0]) / len(trades) * 100
        ) if trades else 0
    ...
    def get_monthly_matrix(user_id, months_limit=6):
        db = Database()
        trades = db.get_recent_trades_summary(user_id, limit=1000)
        workers = db.get_workers(user_id)
        ...
        for w in workers:
            ...
            for m in months:
                m_trades = [
                    t for t in trades
                    if t.get('worker_id') == w['id']
                    and t.get('exit_at')
                    and t['exit_at'].startswith(m)
                ]
                row["data"][m] = sum(float(t.get('result', 0)) for t in m_trades)
            matrix.append(row)
