...
const SyncCounter = ({ marketId }) => {
  const [syncInfo, setSyncInfo] = useState({ count: 0, total: 0, last: null })
  ...
  useEffect(() => {
    const load = async () => {
      setLoading(true)
      try {
        // Get symbols for this market from whitelist
        ...
        if (!wlSymbols || wlSymbols.length === 0) {
  ...
  const pct = syncInfo.total > 0 ? Math.round((syncInfo.count / syncInfo.total) * 100) : 0
  const barColor = pct >= 80 ? '#00FFCC' : pct >= 40 ? '#d4af37' : '#ff3366'
    ...
    >
      <div className="d-flex align-items-center justify-content-between mb-2">
        <div className="d-flex align-items-center gap-2">
          ...
          <span className="small fw-bold" style={{ color: '#d4af37' }}>مزامنة البيانات التاريخية</span>
        </div>
        {loading ? (
          <span className="spinner-border spinner-border-sm" style={{ width: 14, height: 14, color: '#d4af37' }} />
        ) : (
      ...
      {!loading && (
        <>
          <div className="progress mb-2" style={{ height: 6, background: 'rgba(255,255,255,0.08)', borderRadius: 4 }}>
            <div
              className="progress-bar"
              style={{ width: `${pct}%`, background: barColor, borderRadius: 4, transition: 'width 0.6s ease' }}
            />
          </div>
          <div className="d-flex justify-content-between extra-small text-secondary">
            <span>
              <span className="text-white fw-bold">{syncInfo.count}</span> / {syncInfo.total} عملة مزامنة
            </span>
            {syncInfo.last && (
              <span>
                آخر تحديث:{' '}
                <span className="text-white">
                  {new Date(syncInfo.last).toLocaleString('ar-EG', { dateStyle: 'short', timeStyle: 'short' })}
                </span>
              </span>
            )}
          </div>
    ...
    <div className="glass-card" style={{ borderTop: `3px solid ${typeColor}` }}>
      ...
      {expanded && (
        <div className="border-top p-4" style={{ borderColor: 'rgba(255,215,0,0.1)' }}>
          {/* Sync Counter */}
          <div className="mb-4">
            <SyncCounter marketId={market.id} />
          </div>
