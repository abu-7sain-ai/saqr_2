...
# ✅ الـ columns دي مش موجودة في جدول workers في Supabase — نشيلهم قبل أي insert
...
def _sanitize_for_json(obj):
    """
    ✅ FIX: يحول nan/inf/-inf لقيم JSON-safe قبل الإرسال لـ Supabase.
    السبب: numpy بتولّد nan/inf في الـ backtest وده بيكسر JSON serialization.
    """
...
class Database:
    ...
    def update_session_data(session_id, additional_data):
        ...
        try:
            ...
            for key, value in additional_data.items():
                ...
                elif key == "expert_opinions":
                    # FIX: expert_opinions يتحفظ مباشرة — دمج مع القديم عشان ما نمسحش الـ rounds
                    if isinstance(value, dict) and isinstance(metadata_payload, dict):
            ...
            # ✅ FIX: نظّف nan/inf قبل الإرسال لـ Supabase (بيسبب "Out of range float values")
            update_payload = _sanitize_for_json(update_payload)
    ...
    def get_recent_trades_summary(user_id, limit=50):
        ...
        # ✅ FIX: guard against None user_id + removed bad .is_() usage
        if not user_id or str(user_id).lower() == 'none':
            return []
        try:
    ...
    def get_workers(user_id):
        """Alias for get_workers_by_user — used by analytics_service."""
        return Database.get_workers_by_user(user_id)
    ...
    # ✅ FIX 3: Added missing get_whitelist method used by data_connector.py
    @staticmethod
