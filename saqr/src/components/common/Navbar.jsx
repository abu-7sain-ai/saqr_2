  ...
  useEffect(() => {
    const fetchStatus = async () => {
      try {
        ...
        if (mState) setMarketType(mState.current_type)
      } catch (err) {
    ...
    <nav className="navbar navbar-expand-lg mb-4 p-0 animate-fade-in">
      <div className="container-fluid p-0">
        ...
        <div className="d-flex align-items-center gap-3 w-100">
          {/* Market Status Indicator (Responsive) */}
          <div className="glass-card d-flex align-items-center gap-3 px-3 py-2 overflow-hidden">
            <div className="d-flex align-items-center gap-2">
              <span
                className={`pulse-dot ${marketType === 'stable' ? 'bg-emerald' : 'bg-warning'}`}
              ></span>
              <span className="small fw-semibold text-uppercase tracking-wider d-none d-sm-inline">
                {marketType === 'stable' ? 'سوق مستقر' : 'سوق متوتر'}
              </span>
              {connection.connected && (
                <span className="badge bg-accent-primary text-black extra-small ms-2 fw-black px-2">LIVE</span>
              )}
            </div>
            <div className="vr opacity-25 d-none d-sm-block" style={{ height: '16px' }}></div>
            <div className="d-flex align-items-center gap-2">
              {connection.connected ? (
                <>
                  <Wifi size={14} className="text-emerald" />
                  <span className="small text-emerald fw-bold">متصل</span>
                </>
              ) : (
                <>
                  <WifiOff size={14} className="text-ruby opacity-50" />
                  <span className="small text-ruby opacity-75">غير متصل</span>
                </>
              )}
            </div>
          </div>
          ...
          <div className="ms-auto d-flex align-items-center gap-2 gap-md-3">
            ...
            <div className="glass-card d-flex align-items-center gap-2 gap-md-3 px-2 px-md-3 py-1 hover-bg-light cursor-pointer border-0">
              <div className="text-end d-none d-md-block">
                <div className="fw-bold small">أحمد</div>
