...
const StatCard = ({ title, value, icon, change, isPositive, loading }) => (
  ...
  >
    <div className="d-flex justify-content-between mb-4 align-items-start">
      <div 
        className="rounded-4 d-flex align-items-center justify-content-center text-accent-primary shadow-accent"
        style={{ 
          width: '48px', 
          height: '48px', 
      ...
      {!loading ? (
        change && (
          <div
            className={`px-3 py-1 rounded-pill small fw-bold d-flex align-items-center gap-2 ${isPositive ? 'bg-accent-primary bg-opacity-10 text-accent-primary' : 'bg-ruby bg-opacity-10 text-ruby'}`}
          >
        ...
        <div className="placeholder-glow">
          <span className="placeholder rounded-pill bg-accent-primary opacity-25" style={{ width: '60px', height: '24px' }}></span>
        </div>
      )}
    </div>
    <div className="small text-secondary fw-bold opacity-50 mb-2 text-uppercase tracking-widest">
      {title}
    </div>
    {loading ? (
      <div className="placeholder-glow">
        <span className="placeholder col-8 bg-accent-primary opacity-25 rounded-pill"></span>
      </div>
    ) : (
      <div className="h2 m-0 fw-black text-white" style={{ letterSpacing: '-1px' }}>
        {value}
      </div>
    )}
  </div>
